...
- [x] **Task 10: Scroll & clip horizontal** (`UF.java` baris 436–440, 638) — **wrap multi-baris TIDAK dikerjakan**, lihat catatan.
  - `HyperCalDisplayView.onDraw`: `canvas.clipRect(0,0,viewWidth,viewHeight)` sebelum menggambar, supaya ekspresi yang lebih lebar/tinggi dari display tidak bocor ke area keypad.
  - Scroll horizontal sekarang mengikuti posisi kursor (mirip caret-follow pada text field), bukan cuma "selalu geser ke akhir" seperti sebelumnya: `startX` dihitung dari posisi kursor di koordinat lokal (sebelum offset), lalu di-clamp supaya kursor selalu ada di antara `paddingLeft` dan `viewWidth-paddingRight`. Nilai `startX` disimpan lintas frame (`lastStartX`) supaya tidak "loncat" saat digambar ulang.
  - Uji emulator: ketik ekspresi panjang (`123+456+789+123+456+789`) → tergeser otomatis ke akhir, kursor terlihat di tepi kanan. Tekan ◀ berulang kali → tergeser balik mengikuti kursor sampai ke awal (`123|+456+...`), konten tidak tumpah ke keypad.
  - **TIDAK DIKERJAKAN: wrap multi-baris.** Algoritma pembungkusan baris asli (`C0329vH.mo63HiPER()`, ~baris 90–220) jauh lebih kompleks dari scroll horizontal — ada logika ukur lebar per elemen, keputusan pindah baris berdasarkan lebar tersisa (`f3`), dan re-layout starting `i11` per baris. Porting penuh ini di luar cakupan sample ini; scroll horizontal dipilih sebagai solusi praktis yang cukup untuk kebutuhan kalkulator biasa.
- [x] **Task 11: Bersihkan Javadoc usang** di `HyperCalDisplayView` (masih menyebut `22sp`, `R.font.math_symbols`).
- [ ] **Task 12: String ukur `HcZgWQ.LiVE`** pada `AbstractC0335wD.HiPER(paint, f)` — **TIDAK BISA DIPASTIKAN, diblokir tooling**.
  - `HcZgWQ` adalah kelas pool string hasil obfuscator (puluhan field `public static String` dengan nama acak, tanpa nilai literal di Java hasil decompile) — nilainya kemungkinan diisi lewat native library (JNI) saat aplikasi berjalan, bukan lewat bytecode Java biasa. Tidak ada file `.smali` maupun tool `apktool`/`baksmali` di lingkungan ini untuk menelusuri lebih lanjut (sama seperti Task 18).
  - Asumsi `" "` (spasi) dipertahankan berdasarkan pola pemakaian (nilainya dipakai untuk gap/ketebalan relatif kecil seperti 0.2f/0.3f/0.35f, konsisten dengan mengukur lebar satu spasi), tapi ini **tetap tebakan**, bukan fakta terverifikasi.

### F. Pemindahan Lokasi Kode (2026-09-26): `HyperCalDisplayView.java` dipecah
`HyperCalDisplayView.java` sebelumnya 437 baris, mencampur setup View/touch/blink dengan matematika skala font dan pencarian node di pohon visual. Dua bagian dipindah ke class baru (sekarang 364 baris):

- **`view/HyperCalScale.java`** (baru, package-private) — semua isi Task 7: `lineHeight`, `referenceDisplayHeight`, `computeTextSize` (gabungan rumus `f`/`f2`/clamp `1.2×f` yang dulu ada langsung di `updatePaintSize`), plus konstanta `REF_KEYPAD_WIDTH/HEIGHT`, `DISPLAY_PADDING`, `EDITOR_LINES`, `NOMINAL_BASE_SIZE`. Class murni matematika (`Paint`/`Typeface` saja, tidak ada `View`/`Context`), jadi kalau mau memverifikasi ulang rumus skala (mis. setelah Task 12 di atas terjawab), cukup baca file ini tanpa perlu menelusuri `HyperCalDisplayView`.
- **`render/VisualTree.java`** (baru) — isinya method `findVisualForNode` yang lama (kini `VisualTree.find`), dipakai untuk mencari `MathVisual` dari sebuah `ExpressionNode` di pohon hasil `VisualTreeBuilder`. Dipindah ke package `render/` karena sifatnya sama seperti `VisualTreeBuilder` yang sudah ada di sana (murni tahu bentuk pohon visual: `SequenceVisual`, `FractionVisual`, `PowerVisual`, dst.), bukan sesuatu yang spesifik ke `View` Android.
- **`HyperCalDisplayView.java`** (tersisa) — sekarang isinya murni tanggung jawab View: setup touch/blink (`init`, `handleTapAt`, `onTouchEvent`), `onDraw` (layout, scroll, clip, gambar), dan `findVerticalCursorTarget` (Task 12 di `btn_fraction.md`) yang tetap di sini karena butuh hasil `calculateLayout` (Paint) milik View ini.

**Kalau mencari kode terkait task di atas sekarang:** rumus skala Task 7 ada di `view/HyperCalScale.java`, bukan lagi di `HyperCalDisplayView`. Pencarian node-ke-visual (dipakai gambar kursor & Task 12 navigasi atas/bawah di `btn_fraction.md`) ada di `render/VisualTree.java`.
